function AddValue( lat, lon, nome, desc, postiDisp, totPosti ) {  
...
ListObj=new Array();
	//ListGoogleObj=new Array();
	ListObj[0]= new AddValue('44.858669471844436','10.058564504349347','Bar Mario', 'Bar rinomato per i caffè', '7','10');
	//ListGoogleObj = new google.maps.LatLngBounds('44.858669471844436','10.058564504349347');
	ListObj[1]= new AddValue('44.85873031279572','10.088347753250641','Ristorante La Piazza', 'Specialità di pesce e carne', '0','40');
	//ListGoogleObj = new google.maps.LatLngBounds('44.85873031279572','10.088347753250641');
	ListObj[2]=new AddValue('44.84832557566559','10.124139150345277','Pizzeria Bella Napoli', 'Pizzeria aperta 7/7', '1','100');
	//ListGoogleObj = new google.maps.LatLngBounds('44.84832557566559','10.124139150345277');
...
function ChangeDist(valDist){
	
	StoreInPhone('dist',valDist);
	...
	map.addMarker({
	 ...
	 var distVal;
	  if(GET_StoreInPhone('dist'))
	  {
		cir.setRadius(parseInt(GET_StoreInPhone('dist')*'1000'));
		distVal=GET_StoreInPhone('dist');
	  }
	  else
	  {
		cir.setRadius(1000);
	...
	{
		if(GET_gps_distance(latBkup,lonBkup,ListObj[ii].lat,ListObj[ii].lon)<distVal)
		{
		  map.addMarker({
			lat: ListObj[ii].lat,
			lng: ListObj[ii].lon,
...
latBkup=position.coords.latitude;
lonBkup=position.coords.longitude;
   /* var element = document.getElementById('geolocation1');
	...
	var map = new GoogleMap(position.coords.latitude,position.coords.longitude,ListObj);
	map.initialize();*/
	...
	for(var ii=0;ii<ListObj.length;ii++)
	{
		if(GET_gps_distance(position.coords.latitude,position.coords.longitude,ListObj[ii].lat,ListObj[ii].lon)<distVal)
		{
		  map.addMarker({
			lat: ListObj[ii].lat,
			lng: ListObj[ii].lon,
